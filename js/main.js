...
// IMPORTANT: load and register scenarios (side effects).
...
function renderAllUI(){ renderAttackers(StateMod.State); renderTargets(StateMod.State); renderVulns(StateMod.State); populateSelectors(StateMod.State); hydrateEntriesSelect(StateMod.State); renderLinksInspector(); renderDetailsPanel(); }

function filterOnlyVuln(results){ const only = el('chkOnlyVuln')?.checked; if(!only) return results; return results.filter(p => (p.vulnsPerNode||[]).every(v => Array.isArray(v) && v.length>0)); }

function renderResultsView(results, meta){
  const container = el('results'); container.innerHTML = '';
  if(!results.length){ container.innerHTML = `<div class="small">No paths.</div>`; renderStatus('0 paths'); return; }
  results.forEach(p=>{
    const row=document.createElement('div'); row.className='path';
    const left=document.createElement('div'); left.className='left';
    left.innerHTML = `<div><strong>${p.attackerName || p.attacker || ''}</strong></div>
    ...
    btn.onclick=()=>{ lastDiagramSVG=buildSVGForPath(p, StateMod.State); const box=el('diagramBox'); if(box) box.innerHTML=lastDiagramSVG; };
    right.appendChild(btn); row.append(left,right); container.appendChild(row);
  });
  const parts=[`${results.length} paths`]; if(meta.cycles) parts.push('(cycles detected)'); if(meta.truncated) parts.push('(truncated)'); renderStatus(parts.join(' '));
}
...
async function onComputePaths(){
  ...
  }catch(e){ console.error(e); alert('Path computation error.'); }
}

function onExportODS(){ if(!lastResults.length) return alert('No paths to export.'); exportODS(lastResults, { state: StateMod.State }); }
function onDownloadSVG(){ if(!lastDiagramSVG) return alert('No diagram'); const blob=new Blob([lastDiagramSVG],{type:'image/svg+xml'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='diagram.svg'; document.body.appendChild(a); a.click(); a.remove(); setTimeout(()=>URL.revokeObjectURL(a.href),0); }
  ...
  if(btnSimu){
    btnSimu.onclick = async () => {
      try{
        ...
        btnSimu.textContent='Simulating…'; btnSimu.disabled=true;
        await runSimulation({ renderCallback: () => renderAllUI() });
      } finally {
        btnSimu.textContent='Simulation'; btnSimu.disabled=false;
        enableTopButtons(true); renderAllUI();
      }
    };
  }
