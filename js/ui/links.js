...
export function renderLinksInspector(){
  ...
  const makeGroup = (label, mapObj, typeKey) => {
    ...
    const chips = items.map(toId => {
      ...
      const btn = `<button data-type="${typeKey}" data-to="${toId}" class="ghost" style="padding:2px 6px;border-radius:6px">❌</button>`;
  ...
  box.innerHTML = [
    `<div class="mini">Links from <strong>${nameOfTarget(src)}</strong> (click ❌ to remove)</div>`,
...
/* =========================
   Public: wire UI events (buttons)
========================= */

export function wireLinksUI(){
  const btnAdd = el('btnAddLink');
  const btnDel = el('btnRemoveLink');
  const srcSel = el('linkSource');
  const dstSel = el('linkDest');
  const typeSel = el('linkType');

  if(!btnAdd || !btnDel || !srcSel || !dstSel || !typeSel) return;

  // When source changes, refresh inspector
  srcSel.addEventListener('change', ()=> renderLinksInspector());

  // Add links
  btnAdd.onclick = () => {
    const from = srcSel.value;
    if(!from) { alert('Source required'); return; }
    const tos = [...dstSel.selectedOptions].map(o => o.value);
    if(!tos.length) { alert('Destination required'); return; }
    const type = typeSel.value;

    tos.forEach(to => addLink(type, from, to));
    saveToLocal(State);
    renderLinksInspector();
  };

  // Remove links (bulk remove from current selection)
  btnDel.onclick = () => {
    const from = srcSel.value;
    if(!from) { alert('Source required'); return; }
    const tos = [...dstSel.selectedOptions].map(o => o.value);
    ...
    tos.forEach(to => removeLink(type, from, to));
    saveToLocal(State);
    renderLinksInspector();
  };
}
